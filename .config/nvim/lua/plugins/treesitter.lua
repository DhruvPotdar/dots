-- nvim-treesitter: parser manager only.
-- Highlight, folds, and incremental selection are handled by vim.treesitter built-ins.
-- Move/swap keymaps are handled by nvim-treesitter-textobjects.
return {
	{
		...
		config = function(_, opts)
			...
			-- ── Built-in vim.treesitter features ────────────────────────────────
			vim.api.nvim_create_autocmd("FileType", {
				callback = function(args)
					local buf = args.buf

					-- Highlight (built into neovim, queries provided by nvim-treesitter)
					pcall(vim.treesitter.start, buf)

					-- Folds — treesitter expr; start fully open
					local wo = vim.wo[0][0]
					wo.foldmethod = "expr"
			...
			-- <C-space> normal: start visual selection at current node.
			-- <C-space> visual: expand selection to parent node.
			-- <bs>      visual: shrink back to previous selection.
			do
				local sel_stack = {}
			...
			-- ── Textobject move/swap keymaps (nvim-treesitter-textobjects) ───────
			...
			if not ok_move or not ok_swap then return end
